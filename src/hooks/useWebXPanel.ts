...
const useWebXPanel = (params: WebXPanelConfig) => {
  ...
  useEffect(() => {
    ...
    const config: Partial<
      ReturnType<typeof window.WebXPanel.getWebXPanel>["WebXPanelConfigParams"]
    > = params;
    ...
    if (isActive) {
      ...
      const errorWsListener = ({ detail }: any) => {
        console.log(
          `WebXPanel websocket connection error: ${JSON.stringify(detail)}`
        );
      };
      ...
      const connectCipListener = () => {
        ...
        const cipConnectEvent = new CustomEvent("cipConnectedEvent");
        document.dispatchEvent(cipConnectEvent);
      };

      const authenticationFailedListener = ({ detail }: any) => {
        console.log(
          `WebXPanel authentication failed: ${JSON.stringify(detail)}`
        );
      };
      ...
      const disconnectWsListener = ({ detail }: any) => {
        console.log(
          `WebXPanel websocket connection lost: ${JSON.stringify(detail)}`
        );
      };
      ...
      const disconnectCipListener = ({ detail }: any) => {
        ...
        const cipDisconnectEvent = new CustomEvent("cipDisconnectedEvent");
        document.dispatchEvent(cipDisconnectEvent);
      };
