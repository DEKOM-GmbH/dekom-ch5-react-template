...
import { Mic01Icon, MicOff01Icon, VolumeHighIcon, VolumeOffIcon } from '@hugeicons/core-free-icons';
...
export function Button(props:DigitalButtonProps) {
    ...
    const muteType = props.muteType !== undefined ? props.muteType : MuteButtonType.None;
    ...
    return (
        <button id="digitalButton" disabled={disabledState} className={`btn ${digitalState ? (muteType === MuteButtonType.None ? 'selected' : 'muted') : "normal"} ${invisibleState ? "invisible" : ""} ${disabledState ? "disabled": ""}`} onClick={() => sendDigitalClick(props.joinNumber)}>
            {iconDisplay(digitalState, muteType)}{props.children}
        </button>
    );
}
